...
keycap_cs(
  ...
  Stem   = MX, //tusn on shell and stems
...
topthickness = 3.2; //2 for phat 3 for chicago
...
//  BotWid, BotLen, TWDif, TLDif, keyh, WSft, LSft  XSkew, YSkew, ZSkew, WEx, LEx, CapR0i, CapR0f, CapR1i, CapR1f, CapREx, StemEx
    ...
    [17.20,  16.00,   5.6, 	   5,  5.4,    0,   .0,     5,    -0,    -0,   2, 2.5,    .10,      2,     .10,      3,     2,       2], //Chicago Steno R2/R4
    [17.20,  16.00,   5.6, 	   5,  5.0,    0,   .0,     0,    -0,    -0,   2, 2.5,    .10,      3,     .10,      3,     2,       2], //Chicago Steno R3 flat
    [17.20,  16.00,  1.25, 	1.25,  4.8,    0,   .0,     0,    -0,    -0,   2, 2.5,    .10,     .5,     .10,     .5,     2,       2], //Chicago Steno R3 chord
    //mods 3
...
///----- KEY Builder Module
module keycap_cs(keyID = 0, cutLen = 0, visualizeDish = false, crossSection = false, Dish = true, Stem = false, StemRot = 0, homeDot = false, homeBar = false, Stab = 0, Legends = false, fdm = true) {
  $fn = fn;
  ...
  difference(){
    union(){
      ...
      if(Stem == Choc){
        ...
        translate([0,0,(KeyHeight(keyID)-topthickness-2.8)])rotate(stemRot)difference(){
          cylinder(d=5.4,3.0,$fn= 64);
          skin(StemCurve);
          skin(StemCurve2);
        }
      }
   ...
   //Dish Shape
    if(Dish == true){
     ...
     if(fdm == true){
      h=1.9; //height of the cutting cube
